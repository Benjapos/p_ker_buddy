...
for date, game_data in game_groups:
    ...
    if len(redemptions) > 0:
        ...
        if len(winners_data) > 1 and winners_data[0][1] == winners_data[1][1]:
            ...
            notes = f"Split pot with {', '.join(split_winners)}"
        else:
            ...
            if len(winners_data) > 1:
                ...
                is_split = True
                notes = f"Split pot with {', '.join([technical_winner] + other_winners)}. Technical winner: {technical_winner}"
            else:
                # Single winner
    ...
    # Create row with all players (excluding winners from player columns)
    row = {
    ...
    # Add all players to separate columns (excluding winners)
    non_winner_participants = [p for p in all_participants if p not in winners]
    
    for i, player in enumerate(non_winner_participants, 1):
        row[f'player{i}'] = player
    
    # Fill remaining player columns with empty strings
    for i in range(len(non_winner_participants) + 1, 9):  # Up to player8
        row[f'player{i}'] = ''
